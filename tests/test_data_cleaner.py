...
class TestDataCleaner(unittest.TestCase):
    ...
    def test_drop_invalid_rows_removes_rows_with_missing_values(self):
        ...
        cleaned = cleaner.drop_invalid_rows(df, ["name", "age"])
    ...
    def test_trim_strings_strips_whitespace_without_changing_other_columns(self):
        ...
        """
        df = make_sample_df().dropna(subset=["name"]).reset_index(drop=True)
        cleaner = DataCleaner()
        ...
        # DataFrame original conserva los espacios
        self.assertEqual(df.loc[0, "name"], original_names.loc[0])
        self.assertEqual(df.loc[2, "name"], original_names.loc[2])

        # Resultado sin espacios alrededor
        self.assertEqual(result.loc[0, "name"], "Alice")
        self.assertEqual(result.loc[2, "name"], "Carol")

        # Columnas no especificadas permanecen iguales
